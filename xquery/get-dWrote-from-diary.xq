...
let $type:= upper-case(request:get-parameter("type", "write"))
let $diary:= collection('/db/diary')
return
<table xmlns="http://www.tei-c.org/ns/1.0">
<head type="dWroteListing" subtype="{$type}">Writing marked as: {$type}</head>
<row rend="label"><cell>Diary Date</cell><cell>Diary Entry</cell></row>
{   for $text in $diary//tei:ref[@type='dWrote'][lower-case(@subtype)=lower-case($type)]
