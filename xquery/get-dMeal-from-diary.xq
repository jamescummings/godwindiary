...
let $type:= upper-case(request:get-parameter("type", "D"))
let $diary:= collection('/db/diary')
return
<table xmlns="http://www.tei-c.org/ns/1.0">
<head type="mealListing" subtype="{$type}">Meals marked as: {$type}</head>
<row rend="label"><cell>Diary Date</cell><cell>Diary Entry</cell></row>
{   for $text in $diary//tei:seg[@type='dMeal'][lower-case(@subtype)=lower-case($type)]
